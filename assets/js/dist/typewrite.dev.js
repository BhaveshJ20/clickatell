...
(function ($) {
  $.fn.typewrite = function (options) {
    ...
    var settings = $.extend({
      ...
      selectedText: '#333333',
      continuous: false
    }, options); // set the blink speed of the cursor

    settings.blinkSpeed = 1000 / settings.blinkSpeed; // add cursor if set to true.
    ...
    settings.mainEl = this; // add a span to hold our text

    settings.el = $(this).children('span')[0]; // set the typing speed

    settings.speed = 1000 / settings.speed; // holds the tags in an array

    var actions = options.actions; // holds the queue of actions

    settings.queue = actions.length; // trigger the 'typewriteStarted' event

    $(settings.mainEl).trigger('typewriteStarted'); // execute the actions
    ...
    }, 500); // adds a wrapper span around given index of characters to mimick selecting the text
    ...
    function select(action, callback) {
      ...
      chars.forEach(function (_char, index) {
        $(settings.el).delay(settings.speed).queue(function (next) {
          ...
          next(); // we are done, remove from queue
    ...
    function remove(remove) {
      ...
      var chars = blankstr.split(''); // default to stepped

      var removeType = typeof remove.type !== 'undefined' ? remove.type : 'stepped'; // if invalid, set to stepped
  ...
  function callTypeWriteFn() {
    try {
      $("#typewriteText").typewrite({
        ...
        {
          type: "Customer Delight with Clickatell"
        }, {
          delay: 1000
        }, {
          select: {
        ...
        {
          type: "Conversions with Clickatell"
        }, {
          delay: 1000
        }, {
          select: {
